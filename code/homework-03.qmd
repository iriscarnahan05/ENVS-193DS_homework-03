...
```{r}
#| label: QQ-plot-normality-temp
# base layer: ggplot
ggplot(data = temp_kelp, # starting data frame
       aes(sample = temp_c)) + # y-axis for QQ plot
  # first layer: QQ reference line
  geom_qq_line(color = "cyan") +
  # second layer: QQ plot
  geom_qq()
```
...
I checked the assumptions of linearity using a scatter plot and for normality using two Q-Q plots, one for each variable. Given the two Q-Q plots, I would say that both relationships are close enough to be considered normal, as there are only minor deviations from the theoretical distributions, and they exist at the beginning or end of the theoretical line which is not uncommon. There is also a mostly linear relationship shown by the general downward pattern of the scatterplot.
...
ggplot(data = my_hist,
  ...
  labs(title = "Comparison of total time spent biking throughout the \nweek",
...
ggplot(data = class_num,
  ...
  labs(title = "Most biking occurs on average when there are three classes per day",
       subtitle = "May 22, 2026",
